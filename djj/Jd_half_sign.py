...
def JD_shaking():
   ...
   try:
      ...
      response=requests.get(Boxurl,headers=headers)
      #print(response.text)
      ...
      if(Boxres['success']==True):
         ...
         if(Boxres['data']['freeTimes']>0):
            print('shaking go')
            response=requests.get(Skurl,headers=headers)
           ...
           LFtimes=f'''剩余免费{Boxres['data']['freeTimes']}次-不免费{Boxres['data']['feeTimes']}次---总京豆{Boxres['data']['totalBeanCount']}'''
           print(LFtimes)
...
def JD_turncardsign():
   try:
     print('\n京东-首页营销日历签到翻牌')
     hd=headers
     hd['Referer']='https://prodev.m.jd.com/mall/active/36V2Qw59VPNsuLxY84vCFtxFzrFs/index.html?tttparams=1one1jeyJnTG5nIjoiMTEzLjYyNzUyNSIsImdMYXQiOiIyMy4yODA1NjMifQ6%3D%3D&lng=113.274286&lat=23.098026&sid=56f2ed088321ffc84599ff6733ed5e5w&un_area=19_1601_50284_50451'
     hd['Origin']='https://prodev.m.jd.com'
     
     response=requests.post('https://jdjoy.jd.com/api/turncard/channel/sign',headers=hd,data='turnTableId=516')
     res=response.text
     print(res)
...
def JD_Car1209Sign():
   try:
     print('\n京东-中文汽车签到')
     hd=headers
     hd['ActivityId']='f4f998e5b91d48ce957983fe61b0ae90'
     response=requests.post('https://cgame-stadium.jd.com/api/v1/sign',headers=hd)
     res=response.text
     print(res)
     return res
   except Exception as e:
...
def JD_Turn():
   try:
     print('\n京东商城-转盘查询')
     response=requests.post('https://api.m.jd.com/client.action?functionId=wheelSurfIndex&body=%7B%22actId%22%3A%22jgpqtzjhvaoym%22%2C%22appSource%22%3A%22jdhome%22%7D&appid=ld',headers=headers)
...
def JD_TurnSign(code):
   try:
     print('\n京东商城-转盘签到')
     response=requests.post(f'''https://api.m.jd.com/client.action?functionId=lotteryDraw&body=%7B%22actId%22%3A%22jgpqtzjhvaoym%22%2C%22appSource%22%3A%22jdhome%22%2C%22lotteryCode%22%3A%22{code}%22%7D&appid=ld''',headers=headers)
     res=response.text
     print(res)
   except Exception as e:
      print(str(e))
...
def JD_Overseas():
   try:
     ...
     response=requests.post('https://api.m.jd.com/client.action?functionId=checkin',headers=headers,data=body)
     res=response.text
     print(res)
   except Exception as e:
      print(str(e))
...
def JD_Webcasts():
   try:
     print('\n京东商城-直播签到')
     header=headers
     header['Origin']='https://h.m.jd.com'
     response=requests.get('https://api.m.jd.com/api?functionId=getChannelTaskRewardToM&appid=h5-live&body=%7B%22type%22%3A%22signTask%22%2C%22itemId%22%3A%221%22%7D',headers=header)
     res=response.text
     print(res)
   except Exception as e:
      print(str(e))
...
def JD_supermarket():
   try:
     print('\n京东-超市签到')
     response=requests.get('https://api.m.jd.com/api?appid=jdsupermarket&functionId=smtg_sign&clientVersion=8.0.0&client=m&body=%7B%7D',headers=headers)
     res=response.text
     print(res)
   except Exception as e:
      print(str(e))
def JD_SecKilling():
   try:
     ...
     response=requests.post('https://api.m.jd.com/client.action',headers=header,data=body)
     res=json.loads(response.text)
     print(res)
...
def JR_Rebates():
   try:
     print('\n金融金币乐园')
     header=headers
     header['Referer']= 'https://member.jr.jd.com/gcmall/?channel=01-qddlq-201010'
     response=requests.get('https://ms.jr.jd.com/gw/generic/hy/h5/m/receiveRebates?reqData=%7B%22version%22:%221.0%22,%22channel%22:%22%22,%22timeStamp%22:1606817564072%7D',headers=header)
     res=response.text
     print(res)
     return res
   except Exception as e:
...
def JRJT_Checkin():
   try:
     ...
     response=requests.post('https://ms.jr.jd.com/gw/generic/zc/h5/m/openScreenReward',headers=headers,data=body)
     res=response.text
     print(res)
   except Exception as e:
      print(str(e))
def JR_Steel():
   try:
     ...
     response=requests.post('https://ms.jr.jd.com/gw/generic/gry/h5/m/signIn',headers=headers,data=body)
     res=response.text
     print(res)
   except Exception as e:
      print(str(e))
def JR_DoubleSign():
   try:
     ...
     response=requests.post('https://nu.jr.jd.com/gw/generic/jrm/h5/m/process?',headers=headers,data=body)
     res=response.text
     print(res)
   except Exception as e:
      print(str(e))
...
def JR_Redsign():
   try:
     ...
     response=requests.post('https://api.m.jd.com/client.action?functionId=ccSignInNew',headers=headers,data=body)
     res=response.text
...
def JR_LuckyLottery():
   try:
     ...
     response=requests.post('https://ms.jr.jd.com/gw/generic/syh_yxmx/h5/m/handleSign',headers=headers,data=body)
     res=response.text
     print(res)
   except Exception as e:
      print(str(e))
...
def TotalSteel():
   try:
     print('\n总钢镚查询')
     msg='总钢镚查询:'
     response=requests.get('https://coin.jd.com/m/gb/getBaseInfo.html',headers=headers)
     res=json.loads(response.text)
...
def TotalRed():
   try:
     ...
     response=requests.post('https://api.m.jd.com/client.action?functionId=myhongbao_balance',headers=headers,data=body)
     res=json.loads(response.text)
     #print(res)
...
def TotalSubsidy():
   try:
     ...
     msg='总金贴查询'
     header=headers
     header['Referer']='https://active.jd.com/forever/cashback/index?channellv=wojingqb'
     response=requests.get('https://ms.jr.jd.com/gw/generic/uc/h5/m/mySubsidyBalance',headers=header)
     res=json.loads(response.text)
     #print(res)
...
def TotalMoney():
   try:
     print('\n总现金查询')
     msg='总现金:'
     response=requests.get('https://api.m.jd.com/client.action?functionId=cash_exchangePage&body=%7B%7D&build=167398&client=apple&clientVersion=9.1.9&openudid=1fce88cd05c42fe2b054e846f11bdf33f016d676&sign=762a8e894dea8cbfd91cce4dd5714bc5&st=1602179446935&sv=102',headers=headers)
     res=json.loads(response.text)
     #print(res)
