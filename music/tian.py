...
def check():
   ...
   if "WETCARD_TIAN_COOKIE" in os.environ:
      wetcard_tian_cookie = os.environ["WETCARD_TIAN_COOKIE"]
   if wetcard_tian_cookie:
       for line in wetcard_tian_cookie.split('\n'):
         if not line:
            continue 
         wetcard_tianlist.append(line.strip())
   else:
     print('DTask is over.')
     exit()
...
check()
j=0
for count in wetcard_tianlist:
   j+=1
   print(f'''>>>>>>>>>【账号{str(j)}开始】''')
   wetcard_sign(j,count)
   wetcard_cash(j,count)
pushmsg('tiancard',result)
